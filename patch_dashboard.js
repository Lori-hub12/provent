...
// Inject Tab
const tabHook = '<button class="tab-btn" onclick="switchTab(\'historial\')">Historial</button>';
const newTab = '<button class="tab-btn" onclick="switchTab(\'pasaportes\')">Pasaportes QR</button>';
if (!html.includes('switchTab(\'pasaportes\')')) {
    html = html.replace(tabHook, tabHook + '\n                ' + newTab);
}

// Inject Content Section
const sectionHook = '</main>';
const newSection = `
        <section id="pasaportes" class="dashboard-section" style="display:none;">
