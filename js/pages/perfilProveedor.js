...
async function checkFavoriteStatus(providerId) {
    const user = window.ProVendAuth ? ProVendAuth.getCurrentUser() : null;
    if (!user || user.rol !== 'empresa') return;
    try {
        const res = await ProVendAuth.apiFetch(`${API_BASE}/api/dashboard/empresa/${user.id}/favoritos`);
        if (res.ok) {
            const favs = await res.json();
            isFavorite = favs.some(f => f.id == providerId);
            updateFavoriteBtn();
        }
...
window.toggleFavorite = async function() {
    ...
    if (user.rol !== 'empresa') return alert('Solo las empresas pueden guardar proveedores favoritos.');
    ...
    try {
        ...
        const res = await ProVendAuth.apiFetch(`${API_BASE}/api/favoritos`, {
            ...
            body: JSON.stringify({ empresa_id: user.id, proveedor_id: providerId })
        });
        ...
        if(res.ok) {
            ...
            alert(err.error || 'Error al actualizar favoritos');
        }
        btn.disabled = false;
    } catch(e) { 
        alert(e.message); 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const providerId = urlParams.get('id');
    const btn = document.getElementById('btn-favorite');
    if (btn) btn.addEventListener('click', window.toggleFavorite);
    if (providerId) checkFavoriteStatus(providerId);
});
