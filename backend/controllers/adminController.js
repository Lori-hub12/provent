...
exports.get_admin_empresas = async (req, res) => {
    try {
        const empresas = await dbAll(
            `SELECT u.id, u.nombre, u.email, u.activo, u.created_at, u.ultimo_login,
                    u.company, pe.ciudad_operacion as ciudad, pe.telefono_contacto as telefono
             FROM usuarios u
             JOIN perfiles_empresa pe ON pe.usuario_id = u.id
             ORDER BY u.created_at DESC LIMIT 50`
        );
        res.json(empresas);
    } catch (err) {
...
exports.get_admin_materiales = async (req, res) => {
    try {
        const materiales = await dbAll(
            `SELECT m.*, u.company as proveedor_nombre
             FROM materiales m
             JOIN usuarios u ON m.proveedor_id = u.id
             ORDER BY m.fecha_publicacion DESC LIMIT 100`
        );
        res.json(materiales);
    } catch (err) {
        res.status(500).json({ error: err.message });
...
exports.get_admin_resenas = async (req, res) => {
    try {
        const resenas = await dbAll(
            `SELECT r.*, 
                    ue.company as empresa_nombre, 
                    up.company as proveedor_nombre
             FROM resenas r
             JOIN usuarios ue ON r.empresa_id = ue.id
             JOIN usuarios up ON r.proveedor_id = up.id
             ORDER BY r.fecha_creacion DESC LIMIT 100`
        );
        res.json(resenas);
    } catch (err) {
        res.status(500).json({ error: err.message });
